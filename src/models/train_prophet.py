"""Wrapper: exécute le notebook associé à l'entraînement Prophet.

Ce fichier exécute `notebooks/01_baselines_prophet.ipynb` via nbconvert
...
NOTEBOOK = Path(__file__).resolve().parents[2] / 'notebooks' / '01_baselines_prophet.ipynb'
...
def train_prophet(timestamp: str = None) -> dict:
    if not NOTEBOOK.exists():
        raise FileNotFoundError(f"Notebook not found: {NOTEBOOK}")
    _run_notebook(NOTEBOOK)

    artifacts = Path('models') / 'artifacts'
    # Accept multiple naming conventions: look for any pkl with 'prophet' in name
    candidates = sorted([p for p in artifacts.glob('*.pkl') if 'prophet' in p.name.lower()])
    if not candidates:
